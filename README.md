...
| 模块 | 说明 | 状态 |
|------|------|------|
| **对话系统** | 打字机效果、可调速度、角色名称彩色标牌、旁白模式 | ✅ 已实现 |
| **角色立绘** | 多角色同屏（左/中/右/远左/远右）、表情差分切换、说话角色高亮 | ✅ 已实现 |
| **背景系统** | 背景图切换、过渡效果（fade）、CG 全屏模式 | ✅ 已实现；❌ dissolve 效果未实现 |
| **音频系统** | BGM 循环+淡入淡出切换、角色语音自动播放、独立音量控制 | ✅ 已实现 |
| **选项分支** | 多选项菜单、条件分支、变量/好感度系统 | ✅ 已实现 |
| **存档系统** | 20 个存档槽位、localStorage 持久化、场景快照 | ✅ 已实现 |
| **特效系统** | 樱花/雨/雪粒子效果、画面震动、闪白 | ✅ 已实现 |
| **系统菜单** | 文字速度、自动播放速度、BGM/语音/音效/主音量滑块 | ✅ 已实现；⚠️ 音效音量滑块仅 UI，SE 命令不播放声音 |
| **结局系统** | 多结局条件判定、达成记录、结局列表（★/☆） | ✅ 已实现 |
| **对话回看** | 完整对话记录浏览、角色名称着色 | ✅ 已实现 |
